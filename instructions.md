...
- **Watch-Only Wallet (Advanced, optional)** — attach a Bitcoin output descriptor to a family member. Balances are fetched automatically: from your StartOS **Bitcoin Core** node when it's installed (via its RPC bridge), otherwise from public APIs with automatic fallback (mempool.space, blockstream.info, blockcypher.com, blockchain.info). Supported descriptor types: `wpkh`, `pkh`, `sh(wpkh)`, `tr`, or a bare xpub. This is entirely optional — skip it if you only track holdings manually. **Note:** when using public APIs (no Bitcoin Core installed), the initial balance scan is rate-limited and can take a while — possibly several hours. The dashboard's Health Check shows "still scanning" until it completes.
